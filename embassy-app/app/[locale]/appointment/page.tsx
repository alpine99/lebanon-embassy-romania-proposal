...
export function generateMetadata({ params }: { params: { locale: string } }): Metadata {
  ...
  return buildPageMetadata({
    locale,
    path: "/appointment",
    title: dict.centres.appointment.heading,
    description: dict.centres.appointment.intro,
  });
}
...
export default function AppointmentPage({ params }: { params: { locale: string } }) {
  const locale: Locale = isLocale(params.locale) ? params.locale : "en";
  const dict = getDictionary(locale);
  const c = dict.centres.appointment;
  ...
  return (
    <div className="mx-auto max-w-content px-5 py-12 sm:py-14">
      ...
      <AppointmentHub locale={locale} dict={dict} />
    </div>
  );
}
